...
| `CHAT_WAIT_POLL` | `0.5` | how often a `?wait=` long-poll re-reads the room, in seconds — this is the wake latency, so the p90 is roughly `0.75 x` it. **It is also what carries `?wait=` across workers**: the poll re-reads the room file, so a write on any worker reaches a waiter parked on every other one, and `--workers N` costs latency rather than delivery. Lowering it buys that latency with reads — one waiter costs `2/s` here, `20/s` at `0.05` (~50 ms p90), times `CHAT_MAX_WAITERS_TOTAL` per process. Floored at `0.01`; `0` would spin the wait loop |
